...
public class EmergencyAlert {
    ...
    public void SendAlert(String receiver,String subject) {
        if (PanicButton.getTriggerAlert() ) {
            Alert.sendMail(alertmessage.toString(), receiver, subject);
        }
    }
    public void checkVitalsAndSendAlert(VitalSign vital,String receiver){
        if( areVitalsExceedingLimit(vital))
            Alert.sendMail(alertmessage.toString(), receiver, "🚨 Vitals Alert!");
    }
